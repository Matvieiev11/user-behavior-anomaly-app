...
class RegisterWindow(QWidget):
    def __init__(self, login_window=None):
        ...
        self.current_language = (
            getattr(login_window, "current_language", "uk")
            if login_window is not None
            else "uk"
        )

        self.current_theme = (
            getattr(login_window, "current_theme", "dark")
            if login_window is not None
            else "dark"
        )
    ...
    def update_language(self):
        self.setWindowTitle(self.tr("register_window_title"))

        self.language_title.setText(self.tr("language"))
        self.theme_title.setText(self.tr("theme"))

        self.title_label.setText(self.tr("create_account"))
        self.subtitle_label.setText(self.tr("register_subtitle"))

        self.email_input.setPlaceholderText(self.tr("email"))
        self.password_input.setPlaceholderText(self.tr("password"))

        self.register_button.setText(self.tr("register"))
        self.back_button.setText(self.tr("back_to_login"))
    ...
    def handle_register(self):
        ...
        if success:
            self.show_message(
                self.tr("success"),
                self.tr("account_created_successfully").format(
            ...
            self.show_message(
                self.tr("registration_error_title"),
                result,
                QMessageBox.Warning
            )
    ...
    def go_back(self):
        if self.login_window is not None:
            self.login_window.current_language = self.current_language
            self.login_window.current_theme = self.current_theme

            self.login_window.apply_styles()
            self.login_window.update_language()
            self.login_window.update_language_buttons()
            self.login_window.update_theme_button()
